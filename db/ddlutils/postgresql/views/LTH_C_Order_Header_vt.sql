CREATE OR REPLACE VIEW adempiere.LTH_C_Order_Header_vt
AS SELECT o.ad_client_id,
   ...
   FROM c_order o
     ...
     LEFT JOIN ( SELECT t.c_order_id,
            sum(t.taxamt) AS taxamt
           FROM c_ordertax t
          GROUP BY t.c_order_id) tx ON tx.c_order_id = o.c_order_id
     LEFT JOIN ( SELECT DISTINCT x.c_order_id,
            c.rate
           FROM c_ordertax x
             LEFT JOIN c_tax c ON x.c_tax_id = c.c_tax_id) txr ON txr.c_order_id = o.c_order_id;